...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setMessage({ type: 'error', text: 'パスワードは英数字8文字以上16文字以内で入力してください。' });
    return;
  }
    ...
    try {
      ...
      if (updateError) {
        setMessage({ type: 'error', text: `パスワード変更に失敗しました: ${updateError.message}` });
      } else {
  ...
  if (error instanceof Error) {
    setMessage({ type: 'error', text: `予期せぬエラーが発生しました: ${error.message}` });
  } else {
